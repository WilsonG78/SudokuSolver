from __future__ import annotations
import numpy as np
import os
import math
from CSudokuSolver import solve_dfs as solve_dfs_c, solve_dlx as solve_dlx_c
from typing import List
from math import sqrt




class Grid:
    grid: np.array
    grid_size: int
    box_size: int

    def __init__(self, array: np.array = None, file_name: str = None):
        if file_name is None and array is None:
            return
        if array is None:
            self.init_by_file(file_name)
            return
        else: 
            self.init_by_array(array)
        if not self.validation():
            raise ValueError("Matrix is incorrect")

    def init_by_file(self, file_name: str) -> None:
        input_data = None
        with open(file_name) as f:
            input_data = f.read().strip()
        if not input_data:
            raise ValueError("File which should contain grid is empty")
        rows = input_data.split("\n")
        n = len(rows)
        self.grid_size = n

        try:
            grid_list = [list(map(int, row.split())) for row in rows]
        except Exception as e:
            raise ValueError(f"Error parsing number {e}")

        if any(len(row) != n for row in grid_list):
            raise ValueError(f"Invalid row lenght")
        if math.isqrt(n) ** 2 != n:
            raise ValueError(f"Invalid lenght of ")

        self.grid = np.array(grid_list, dtype=int)
        self.box_size = math.isqrt(n)
        if np.any(self.grid < 0) or np.any(self.grid > self.grid_size):
            raise ValueError(f"Numbers must be between 0 and {self.grid_size}")

    def init_by_array(self, array) -> None:
        if (
            len(array.shape) != 2
            or array.shape[0] != array.shape[1]
            or math.isqrt(array.shape[0]) ** 2 != array.shape[0]
        ):
            raise ValueError("Shape of array is not correct")
        self.grid_size = array.shape[0]
        for value in array.flat:
            if value > self.grid_size or value < 0:
                raise ValueError(f"Bad number value to {value}")
        self.grid = array
        self.box_size = math.isqrt(self.grid_size)
        if np.any(self.grid < 0) or np.any(self.grid > self.grid_size):
            raise ValueError(f"Numbers must be between 0 and {self.grid_size}")

    def validation(self)->bool:
        n= self.grid.shape[0]
        rows = [set() for i in range(n)]
        columns = [set() for i in range(n)]
        boxes = [set() for i in range(n)]
        for (i,j) ,value in np.ndenumerate(self.grid):
            if  value in rows[i] or value in columns[j] or value in  boxes[self.box_id(i,j,n)]:
                print(i,j,value)
                return False
            if value != 0:
                rows[i].add(value)
                columns[j].add(value)
                boxes[self.box_id(i,j,n)].add(value)
        return True

    def box_id(self,i,j,n)->int:
        sub_n = sqrt(n)
        return int((i//sub_n) * sub_n + (j //sub_n)) 


    def __str__(self) -> str:
        n = self.grid.shape[0]
        b = int(math.isqrt(n))

        cell_w = max(1, max(len(str(int(x))) for x in self.grid.flatten()))

        def format_row(row: np.ndarray) -> str:
            groups = []
            for start in range(0, n, b):
                cells = [str(int(v)).rjust(cell_w) for v in row[start : start + b]]
                groups.append(" " + " ".join(cells) + " ")
            return "|" + "|".join(groups) + "|"

        sample_row = format_row(np.zeros(n, dtype=int))
        hline = "".join("+" if ch == "|" else "-" for ch in sample_row)

        lines = [hline]
        for i in range(n):
            lines.append(format_row(self.grid[i]))
            if (i + 1) % b == 0:
                lines.append(hline)
        return "\n".join(lines)

    def solve_dlx_grid(self) -> list[Grid]:
        solutions = solve_dlx_c(self.grid)
        return [Grid(solution) for solution in solutions]

    def solve_dfs_grid(self) -> list[Grid]:
        solutions = solve_dfs_c(self.grid)
        return [Grid(solution) for solution in solutions]
    
    def solve_dlx_np(self)->list[np.array]:
        solutions = solve_dlx_c(self.grid)
        return solutions
    
    def solve_dfs_np(self)->list[np.array]:
        solutions = solve_dfs_c(self.grid)
        return solutions

if __name__ == "__main__":
    new_grid = Grid(file_name="sudokus/1sudoku.txt")
    print(new_grid)
    solutions = solve_dlx_c(new_grid.grid)
    grids = [Grid(solution) for solution in solutions]
    for grid in grids:
        print(grid)
